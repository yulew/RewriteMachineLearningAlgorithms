    ...
    def fit(self, X, y):
        ...
        for m in range(self.n_estimators):
            for k in range(self.n_classes):
                Reg = DT_regressor(max_depth=self.max_depth, min_samples_split=self.min_samples_split,
                ...
                for node in J:
                    gamma = (self.n_classes - 1) / self.n_classes * np.sum(residue[:, k][node_labels == node]) / np.sum(
                        np.abs(residue[:, k][node_labels == node]) * (1 - np.abs(residue[:, k][node_labels == node])))
                    # print(gamma)
